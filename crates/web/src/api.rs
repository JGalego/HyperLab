...
/// The stack as single-file JSON, without touching whether it is dirty.
///
/// What the export module is handed. [`save_stack_json`] cannot serve here:
/// a download *is* the save, so it marks the document clean, and exporting
/// a PDF is not saving the stack.
#[wasm_bindgen]
pub fn stack_text(_args: &str) -> ApiResult {
    with_doc(|doc| {
        let text = hyperlab_persistence::single_file_string(doc.runtime.stack())
            .map_err(|error| error.to_string())?;
