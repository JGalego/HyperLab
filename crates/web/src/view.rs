...
//!
//! A twin of `apps/desktop/src-tauri/src/view.rs`, kept in step by hand: the
//! two shells must send the same snapshot, because they stand behind the
//! same renderer. The frontend never receives a `Stack`. It receives a
//! [`StackView`]: a flat, serializable snapshot of the current card, taken
//! after every change.
//!
...
pub struct StackView {
    ...
    pub dirty: bool,
    /// Where the stack is saved, if it has been saved. A browser has nowhere
    /// to save one, so this stays `None` and the page offers a download
    /// instead.
...
mod tests {
    ...
    #[test]
    fn a_snapshot_reads_like_the_stack_it_was_taken_of() {
        let mut runtime = Runtime::new(Stack::new("Notes"));
        let card = runtime.current_card();
        runtime
            .execute(Command::CreatePart {
                owner: hyperlab_runtime::PartOwner::Card { id: card },
                kind: PartKind::Button,
                name: "Go".into(),
                geometry: Rect::new(10, 20, 100, 30),
            })
            .unwrap();

        let view = snapshot(&runtime, true, None);
        assert_eq!(view.stack_name, "Notes");
        assert_eq!(view.card_count, 1);
        assert_eq!(view.card_number, 1);
        assert!(view.dirty);
        assert_eq!(view.path, None);

        let button = &view.card.parts[0];
        assert_eq!(button.kind, "button");
        assert_eq!(button.name, "Go");
        assert_eq!(button.rect, [10, 20, 100, 30]);
    }

    #[test]
    fn the_snapshot_serializes_in_the_shape_the_renderer_expects() {
        let runtime = Runtime::new(Stack::new("Notes"));
        let json = serde_json::to_value(snapshot(&runtime, false, None)).unwrap();
        // Spot-checked against apps/desktop/src/types.ts, which both shells
        // must satisfy.
        assert_eq!(json["stackName"], "Notes");
        assert!(json["cardSize"]["width"].is_number());
        assert!(json["card"]["parts"].is_array());
        assert_eq!(json["messageBox"], "");
